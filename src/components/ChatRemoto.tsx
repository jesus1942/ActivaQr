...
export const ChatRemoto: React.FC<Props> = ({ mensajes, miRol, onEnviar, cargando }) => {
  ...
  return (
    <div className="flex flex-col border border-slate-900 bg-white" style={{ height: '360px' }}>
      {/* Mensajes */}
          ...
          return (
            <div key={m.id} className={`flex ${esMio ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] ${esMio
                ...
                {!esMio && (
                  <p className="text-xs font-bold uppercase tracking-wider mb-1 text-brand-600">{m.autorNombre}</p>
                )}
                {m.tipo === 'imagen' && m.adjunto ? (
                  <img
                    ...
                    className="max-w-full max-h-48 border border-slate-900 cursor-pointer rounded"
                  />
      ...
      {grabando ? (
        <div className="border-t-2 border-slate-900 flex items-center gap-3 px-3 py-2">
          ...
          <button
            ...
            className="ml-auto flex items-center gap-1 px-3 py-1.5 bg-brand-600 text-white border border-slate-900 font-bold text-sm hover:bg-brand-700"
          >
        ...
        <form onSubmit={handleEnviarTexto} className="border-t-2 border-slate-900 flex">
          ...
          <button
            ...
            disabled={enviando}
            title="Enviar foto"
            className="px-3 border-r-2 border-slate-900 text-slate-700 hover:text-brand-600 disabled:opacity-40"
          >
            <ImagePlus size={16} />
          </button>
          <button
          ...
          <button
            ...
            className="px-4 bg-brand-600 text-white border-l-2 border-slate-900 font-bold disabled:opacity-40 hover:bg-brand-700 transition-colors"
          >
            <Send size={16} />
          </button>
        </form>
