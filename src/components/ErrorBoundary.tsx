...
export class ErrorBoundary extends React.Component<Props, State> {
  ...
  render() {
    ...
    if (variant === 'full') {
      return (
        <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
          <div className="bg-white border-4 border-slate-900 shadow-[8px_8px_0px_0px_rgba(0,0,0,0.8)] max-w-lg w-full">
            ...
            <div className="p-6 space-y-4">
              ...
              <p className="text-xs font-mono text-slate-500 bg-slate-50 border-2 border-slate-200 p-3 break-words">
                {error.message || error.name || 'Error desconocido'}
              </p>
              <div className="flex flex-col gap-2 pt-2">
                <button
                  onClick={() => window.location.reload()}
                  className="w-full flex items-center justify-center gap-2 bg-orange-500 text-white h-12 font-sketch font-black text-base uppercase border-2 border-slate-900 shadow-[4px_4px_0px_0px_#1e293b] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_#1e293b] transition-all"
                >
                ...
                <a
                  ...
                  className="w-full text-center bg-slate-900 text-white font-bold uppercase tracking-wide text-sm border-2 border-slate-900 py-3 hover:bg-slate-700 transition-colors"
                >
                  Avisar por WhatsApp
                </a>
                <button
                  onClick={this.copiarError}
                  className="w-full flex items-center justify-center gap-2 border-2 border-slate-300 text-slate-700 py-2.5 text-sm font-bold hover:border-slate-900 transition-colors"
                >
    ...
    return (
      <div className="bg-white border-2 border-red-400 shadow-[4px_4px_0px_0px_rgba(248,113,113,0.5)] max-w-2xl mx-auto my-6 p-6 space-y-4">
        <div className="flex items-start gap-3">
          <AlertTriangle size={22} className="text-red-600 flex-shrink-0 mt-0.5" />
          <div>
            <h2 className="font-sketch font-black text-xl uppercase tracking-tight text-slate-900">
        ...
        <p className="text-xs font-mono text-slate-500 bg-slate-50 border border-slate-200 p-2.5 break-words">
          {error.message || error.name || 'Error desconocido'}
        </p>
        <div className="flex flex-wrap gap-2 pt-1">
          <button
            onClick={() => this.setState({ error: null, errorInfo: null, copiado: false })}
            className="flex items-center gap-1.5 bg-orange-500 text-white px-4 py-2 font-bold uppercase tracking-wide text-sm border-2 border-slate-900 shadow-[3px_3px_0px_0px_#1e293b] hover:translate-x-[-1px] hover:translate-y-[-1px] transition-all"
          >
          ...
          {this.props.onVolver && (
            <button
              ...
              className="flex items-center gap-1.5 border-2 border-slate-300 text-slate-700 px-4 py-2 font-bold uppercase tracking-wide text-sm hover:border-slate-900 transition-colors"
            >
              <Home size={14} /> Inicio
            </button>
          )}
