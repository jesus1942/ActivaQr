...
export const Reportes: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      <div>
        <h1 className="font-display text-2xl sm:text-3xl font-bold text-content tracking-tight">Reportes</h1>
        <p className="text-muted text-sm mt-1">Genera reportes de mediciones y estado de activos</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <Card className="space-y-4" padding="md">
            <h2 className="font-display text-base font-bold text-content">Parámetros</h2>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="text-xs font-semibold text-muted tracking-wide">Activos</label>
                <button
              ...
              <div className="space-y-1 max-h-64 overflow-y-auto">
                {activos.map((a) => (
                  <label key={a.id} className="flex items-center gap-2 cursor-pointer hover:bg-subtle p-2 rounded-md transition-colors">
                    <input
                      type="checkbox"
                    ...
                    />
                    <span className="text-xs font-mono font-semibold text-content">{a.codigo}</span>
          ...
          {preview ? (
            <Card padding="md">
              ...
              <div className="border-b border-line pb-4 mb-6">
                <h2 className="font-display text-2xl font-bold text-content text-center">ActivaQR</h2>
                <h3 className="text-sm font-semibold text-center text-muted">Reporte de Activos Industriales</h3>
              ...
              <div className="text-sm space-y-6">
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-6 text-center">
                  <div className="flex-1 border border-line rounded-md p-3 bg-subtle">
                    <div className="font-display text-3xl font-bold text-brand-600">{selectedActivos.length}</div>
                    <div className="text-xs text-muted font-semibold mt-1">Activos</div>
                  </div>
                  <div className="flex-1 border border-line rounded-md p-3 bg-subtle">
                    <div className="font-display text-3xl font-bold text-ok-strong dark:text-ok">{filteredMediciones.length}</div>
                    <div className="text-xs text-muted font-semibold mt-1">Mediciones</div>
                  </div>
                </div>
                ...
                {selectedActivos.map((activoId) => {
                  ...
                  return (
                    <div key={activoId}>
                      <div className="bg-subtle border border-line rounded-t-md px-3 py-2 flex justify-between items-center gap-2">
                      ...
                      {meds.length > 0 ? (
                        ...
                        <table className="w-full text-xs border-x border-b border-line rounded-b-md">
                          <thead>
                            <tr className="bg-surface-2 text-faint">
                              <th className="text-left px-2 py-1.5 font-semibold">Fecha</th>
                              <th className="text-left px-2 py-1.5 font-semibold">Temp.</th>
                              <th className="text-left px-2 py-1.5 font-semibold">Amp.</th>
                              <th className="text-left px-2 py-1.5 font-semibold">Estado</th>
                              <th className="text-left px-2 py-1.5 font-semibold">Técnico</th>
                            </tr>
                          </thead>
                        ...
                        <div className="border-x border-b border-line rounded-b-md px-3 py-2 text-faint text-xs">Sin mediciones en el período</div>
                      )}
                    </div>
                  );
                })}
            ...
            <div className="flex items-center justify-center h-64 border border-dashed border-line-strong rounded-lg text-faint bg-surface">
