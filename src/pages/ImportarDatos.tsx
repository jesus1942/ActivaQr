...
COM-XXX-001,Compresor Ejemplo,compresor,Taller,Schulz,CSL 10,Pedro López,Taller Norte`;
  ...
  return (
    <div>
      ...
      {imported && (
        <div className="bg-ok/10 border border-ok p-4 mb-6 flex items-center gap-3">
          <CheckCircle size={20} className="text-ok-strong dark:text-ok" />
          <span className="font-bold text-ok-strong dark:text-ok">Importación completada exitosamente</span>
        </div>
      )}

      {/* Drop Zone */}
      <div
        className={`border border-dashed p-6 sm:p-8 md:p-12 text-center mb-6 transition-colors cursor-pointer ${
      ...
      >
        ...
        <p className="text-lg font-bold text-content">Arrastra tu archivo CSV aquí</p>
        <p className="text-sm text-faint mt-1">o haz clic para seleccionar</p>
        <input
          ref={fileRef}
          type="file"
          accept=".csv"
      ...
      {error && (
        <div className="bg-danger/10 border border-danger p-4 mb-6 flex items-center gap-3">
      ...
      <div className="bg-surface/85 backdrop-blur-xl border border-line shadow-soft p-4 mb-6">
        <div className="flex items-center justify-between gap-2 mb-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-black uppercase tracking-wider text-content">Formato del Archivo CSV</h2>
          </div>
          <button
        ...
        <p className="text-sm text-muted mb-3">El archivo debe tener las siguientes columnas (separadas por comas):</p>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
          {[
            ...
            <div key={col} className={`text-xs font-mono px-2 py-1 border ${req ? 'border-brand-600 bg-brand-50 text-brand-700' : 'border-line bg-subtle text-muted'}`}>
        ...
        <div className="bg-surface/85 backdrop-blur-xl border border-line shadow-soft p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              ...
              className="flex items-center gap-2 bg-brand-600 text-white px-4 py-2 font-bold border border-line shadow-soft"
            >
              <Upload size={15} />
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {preview.map((row, i) => (
                  <tr key={i} className={`border-b border-line ${i % 2 === 0 ? 'bg-surface' : 'bg-subtle'}`}>
