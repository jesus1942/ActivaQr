...
const initialForm = (empresa: EmpresaControlAdmin): Form => ({
  ...
  tableroConfig: empresa.moduloControl?.tableroConfig ?? { subtitulo: 'Supervisión de cadena de frío', refreshSeconds: 15, mostrarBateria: true, mostrarSenal: true },
});
      ...
      {selected && form && <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/60 p-0 backdrop-blur-sm sm:items-center sm:p-4" onMouseDown={(e) => e.target === e.currentTarget && setSelected(null)}>
        <div className="max-h-[94vh] w-full max-w-2xl overflow-y-auto border border-line bg-surface shadow-2xl">
          ...
          <div className="grid gap-5 p-5 sm:grid-cols-2">
            ...
            <label className="text-xs font-black uppercase text-muted">Actualizar cada (segundos)<input type="number" min="5" max="300" value={form.tableroConfig?.refreshSeconds ?? 15} onChange={(e) => setForm({ ...form, tableroConfig: { ...form.tableroConfig, refreshSeconds: Number(e.target.value) } })} className="mt-1 h-11 w-full border border-line bg-surface px-3 text-sm normal-case" /></label>
