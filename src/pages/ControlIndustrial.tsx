...
export const ControlIndustrial: React.FC = () => {
  ...
  return <div className="space-y-5 pb-16">
    ...
    {selectedVariable && <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/60 sm:items-center sm:p-4" onMouseDown={(e) => e.target === e.currentTarget && setSelectedVariable(null)}><div className="w-full max-w-3xl border border-line bg-surface p-5 shadow-2xl"><div className="flex items-start justify-between"><div><p className="text-xs font-black uppercase tracking-wider text-cyan-600">{selectedVariable.device.nombre}</p><h2 className="font-display text-2xl font-black text-content">{selectedVariable.variable.nombre}</h2><p className="text-sm text-muted">Actual: {valueOf(selectedVariable.variable)}</p></div><button onClick={() => setSelectedVariable(null)} className="text-faint">Cerrar</button></div><div className="mt-5 h-64"><ResponsiveContainer width="100%" height="100%"><LineChart data={history.map((item) => ({ hora: new Date(item.medidaEn).toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' }), valor: item.valorNumero }))}><CartesianGrid strokeDasharray="3 3" opacity={0.2} /><XAxis dataKey="hora" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} domain={['auto', 'auto']} /><Tooltip /><Line type="monotone" dataKey="valor" stroke="#0891b2" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer></div></div></div>}
    ...
    {webhook && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4"><div className="w-full max-w-xl border border-line bg-surface p-5 shadow-2xl"><h2 className="font-display text-xl font-black text-content">Endpoint de ingesta creado</h2><p className="mt-2 text-sm text-muted">Copialo ahora en el UG65. Al rotarlo, el anterior deja de funcionar.</p><code className="mt-4 block break-all border border-line bg-subtle p-3 text-xs text-content">{webhook}</code><div className="mt-4 flex gap-2"><button onClick={() => navigator.clipboard.writeText(webhook).then(() => toast('Endpoint copiado.', 'success'))} className="h-11 flex-1 bg-cyan-700 text-xs font-black uppercase text-white">Copiar</button><button onClick={() => setWebhook(null)} className="h-11 border border-line px-5 text-xs font-black uppercase">Listo</button></div></div></div>}
  </div>;
};
...
const ModalShell = ({ title, children, onClose }: { title: string; children: React.ReactNode; onClose: () => void }) => <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/60 sm:items-center sm:p-4" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><div className="max-h-[94vh] w-full max-w-xl overflow-y-auto border border-line bg-surface shadow-2xl"><div className="sticky top-0 flex items-center justify-between bg-slate-950 px-5 py-4 text-white"><h2 className="font-display text-lg font-black">{title}</h2><button onClick={onClose} className="text-xs uppercase text-slate-400">Cerrar</button></div><div className="p-5">{children}</div></div></div>;
...
function CredentialsModal({ integration, onClose, onDone, toast }: { integration: IntegracionIoT; onClose: () => void; onDone: () => void; toast: (message: string, variant?: 'success' | 'error' | 'warning' | 'info') => void }) {
  const [form, setForm] = useState({ appId: '', appSecret: '', accessToken: '', region: 'us' }); const [pollingSeconds, setPollingSeconds] = useState(Number(integration.configuracion?.pollingSeconds) || 300); const [saving, setSaving] = useState(false);
  return <ModalShell title="Credenciales SONOFF / eWeLink" onClose={onClose}><form className="space-y-4" onSubmit={async (e) => { e.preventDefault(); setSaving(true); try { await guardarCredenciales(integration.id, form); await actualizarIntegracion(integration.id, { configuracion: { pollingSeconds } }); toast('Credenciales cifradas y sincronización programada.', 'success'); onDone(); } catch (error) { toast(error instanceof Error ? error.message : 'No se pudieron guardar.', 'error'); } finally { setSaving(false); } }}><p className="text-sm text-muted">Estos datos viajan sólo al servidor. Después de guardar, no se vuelven a mostrar.</p><Field label="App ID"><input required value={form.appId} onChange={(e) => setForm({ ...form, appId: e.target.value })} className={input} autoComplete="off" /></Field><Field label="App Secret"><input required type="password" value={form.appSecret} onChange={(e) => setForm({ ...form, appSecret: e.target.value })} className={input} autoComplete="new-password" /></Field><Field label="Access Token"><textarea required rows={3} value={form.accessToken} onChange={(e) => setForm({ ...form, accessToken: e.target.value })} className={`${input} h-auto py-3`} /></Field><div className="grid grid-cols-2 gap-3"><Field label="Región"><select value={form.region} onChange={(e) => setForm({ ...form, region: e.target.value })} className={input}><option value="us">América</option><option value="eu">Europa</option><option value="as">Asia</option><option value="cn">China</option></select></Field><Field label="Actualizar"><select value={pollingSeconds} onChange={(e) => setPollingSeconds(Number(e.target.value))} className={input}><option value={60}>Cada minuto</option><option value={300}>Cada 5 minutos</option><option value={900}>Cada 15 minutos</option><option value={3600}>Cada hora</option></select></Field></div><p className="border border-amber-500/30 bg-amber-500/5 p-2 text-xs text-muted">Un minuto consume más cuota eWeLink. Para varios clientes conviene APPID empresarial; Milesight por HTTPS no usa esta cuota.</p><button disabled={saving} className="h-12 w-full bg-cyan-700 text-xs font-black uppercase text-white">Guardar de forma segura</button></form></ModalShell>;
}
