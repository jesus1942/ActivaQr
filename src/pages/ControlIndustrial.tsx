  ...
  return <div className="space-y-5 pb-16">
    <header className="relative overflow-hidden border border-slate-800 bg-slate-950 p-5 text-white sm:p-7">
      ...
      <div className="relative flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        <div><div className="mb-2 flex items-center gap-2 text-[11px] font-black uppercase tracking-[.22em] text-cyan-300"><Activity size={15} /> Supervisión en vivo</div><h1 className="font-display text-3xl font-black">{data.modulo.nombreServicio}</h1><p className="mt-1 text-sm text-slate-400">{data.modulo.tableroConfig?.subtitulo || usuario?.empresa?.nombre} · actualización automática cada 5 segundos</p></div>
    ...
    {tab === 'vivo' && <section className="space-y-4">
      {!data.dispositivos.length ? <Empty icon={RadioTower} title="Esperando el primer dispositivo" text="Configurá un conector y enviá la primera lectura. El equipo aparecerá automáticamente en este tablero." action={owner ? () => setTab('conexiones') : undefined} actionLabel="Configurar conexión" /> : <div className="grid gap-4 xl:grid-cols-2">
        {data.dispositivos.map((device) => { const shown = visibleVariables(device); const channels = channelVariables(device); const operable = device.tipo !== 'puente_rf' && channels.length > 0; return <article key={device.id} className={`border bg-surface shadow-soft ${device.estado === 'critico' ? 'border-red-500' : 'border-line'}`}>
          ...
          <div className="flex flex-wrap items-center gap-4 p-3 text-[10px] uppercase tracking-wider text-faint">{data.modulo.tableroConfig?.mostrarBateria !== false && <span>Batería {device.bateria == null ? '—' : `${device.bateria}%`}</span>}{data.modulo.tableroConfig?.mostrarSenal !== false && <span>Señal {device.rssi == null ? '—' : `${device.rssi} dBm`}</span>}<button onClick={() => downloadHistory('device', device.id)} className="ml-auto flex items-center gap-1 font-black text-muted"><Download size={13} /> Exportar 24 h</button>{editable && <button onClick={() => setSettingsFor(device)} className="flex items-center gap-1 font-black text-muted"><Settings2 size={13} /> Configurar</button>}{editable && data.modulo.controlRemotoHabilitado && operable && (device.permiteControl ? <button onClick={() => setCommandFor(device)} className="font-black text-cyan-700">Operar luces</button> : <button onClick={() => setSettingsFor(device)} className="font-black text-amber-600">Habilitar control</button>)}</div>
        </article>; })}
      </div>}
    </section>}
    ...
    {selectedVariable && <DialogViewport className="z-50 flex items-center justify-center bg-slate-950/60 p-2 backdrop-blur-sm sm:p-4" onEscape={() => setSelectedVariable(null)}><div className="max-h-[calc(100dvh-1rem)] w-full max-w-3xl overflow-y-auto border border-line bg-surface p-5 shadow-2xl" role="dialog" aria-modal="true"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-wider text-cyan-600">{selectedVariable.device.nombre}</p><h2 className="font-display text-2xl font-black text-content">{selectedVariable.variable.nombre}</h2><p className="text-sm text-muted">Actual: {valueOf(selectedVariable.variable)} · {history.length.toLocaleString('es-AR')} registros</p></div><button onClick={() => setSelectedVariable(null)} className="text-faint">Cerrar</button></div><div className="mt-4 flex flex-wrap gap-2"><select value={historyHours} onChange={(event) => setHistoryHours(Number(event.target.value))} className="h-10 border border-line bg-surface px-3 text-xs font-bold text-content"><option value={24}>Últimas 24 horas</option><option value={168}>Últimos 7 días</option><option value={720}>Últimos 30 días</option></select><button onClick={() => downloadHistory('variable', selectedVariable.variable.id, historyHours)} className="flex h-10 items-center gap-2 border border-line px-3 text-xs font-black uppercase text-content"><Download size={15} /> Exportar canal</button></div><div className="mt-5 h-64"><ResponsiveContainer width="100%" height="100%"><LineChart data={history.map((item) => ({ hora: new Date(item.medidaEn).toLocaleString('es-AR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }), valor: item.valorNumero ?? (item.valorBooleano == null ? null : item.valorBooleano ? 1 : 0) }))}><CartesianGrid strokeDasharray="3 3" opacity={0.2} /><XAxis dataKey="hora" tick={{ fontSize: 10 }} minTickGap={30} /><YAxis tick={{ fontSize: 10 }} domain={['auto', 'auto']} /><Tooltip /><Line type="monotone" dataKey="valor" stroke="#0891b2" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer></div></div></DialogViewport>}
