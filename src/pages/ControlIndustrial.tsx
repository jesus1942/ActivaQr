...
export const ControlIndustrial: React.FC = () => {
  ...
  return <div className="min-w-0 space-y-4 pb-20 sm:space-y-5 sm:pb-16">
    ...
    {tab === 'vivo' && <section className="flex flex-col gap-4">
      ...
      {!data.dispositivos.length ? <div className="order-1 lg:order-2"><Empty icon={RadioTower} title="Esperando el primer dispositivo" text="Configurá un conector y enviá la primera lectura. El equipo aparecerá automáticamente en este tablero." action={owner ? () => setTab('conexiones') : undefined} actionLabel="Configurar conexión" /></div> : <div className="order-1 grid items-start gap-4 lg:order-2 xl:grid-cols-2">
      ...
      <div className="grid min-w-0 items-start gap-4 sm:gap-5 md:grid-cols-2 2xl:grid-cols-3">
        {data.dispositivos.map((device) => {
          ...
          return <article key={device.id} className="flex min-w-0 flex-col bg-white p-4 shadow-[4px_4px_0_#0f172a] sm:min-h-56 sm:p-6 sm:shadow-[7px_7px_0_#0f172a]">
            ...
            {channels.length > 0 && !isMotorMode(device) && <div className="mt-5 grid gap-2 sm:mt-6 sm:grid-cols-2 sm:gap-3">{channels.slice(0, 4).map((channel, index) => { const canal = Number(channel.clave.slice(7)) - 1; const name = channel.nombre || `Canal ${index + 1}`; const power = channelMetrics(device, index + 1).find((item) => /^(actpow|power)/i.test(item.clave)); return <div key={channel.id} className="min-w-0 bg-[#edf1ef] p-3 sm:p-4"><div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0 flex-1"><strong className="block break-words text-base font-black leading-tight">{name}</strong><span className="mt-1 block break-words text-xs text-slate-500">{channel.valorBooleano ? `Encendida${power ? ` · ${valueOf(power)}` : ''}` : 'Apagada'}</span></div><span className={`mt-1 h-3 w-3 shrink-0 rounded-full ${channel.valorBooleano ? 'bg-amber-400' : 'bg-slate-400'}`} /></div>{operable(device) && <button onClick={() => onCommand(device, { canal, encendido: !channel.valorBooleano, nombre: name })} className={`mt-3 min-h-12 w-full text-xs font-black uppercase sm:mt-4 sm:min-h-11 ${channel.valorBooleano ? 'bg-slate-950 text-white' : 'bg-cyan-300 text-slate-950'}`}>{channel.valorBooleano ? 'Apagar' : 'Encender'}</button>}</div>; })}</div>}
...
function CompactDeviceCard({ device, expanded, remoteEnabled, editable, showBattery, showSignal, onToggle, onHistory, onCommand, onSettings, onExport }: { device: DispositivoIoT; expanded: boolean; remoteEnabled: boolean; editable: boolean; showBattery: boolean; showSignal: boolean; onToggle: () => void; onHistory: (variable: VariableIoT) => void; onCommand: (action?: { canal: number; encendido: boolean; nombre: string }) => void; onSettings: () => void; onExport: () => void }) {
  ...
  return <article className={`min-w-0 overflow-hidden border bg-surface shadow-soft ${device.estado === 'critico' ? 'border-red-500' : 'border-line'}`}><div className="flex min-w-0 items-start gap-3 border-b border-line p-3 sm:p-4"><div className={`grid h-10 w-10 shrink-0 place-items-center border ${stateTone[device.estado] ?? stateTone.sin_datos}`}><DeviceIcon device={device} operable={operable} /></div><div className="min-w-0 flex-1"><h2 className="break-words font-display text-base font-black leading-tight text-content sm:text-lg">{device.nombre}</h2><p className="mt-1 break-words text-xs text-faint">{device.ubicacion || deviceKind(device)}</p></div><div className="shrink-0 text-right"><span className={`inline-block h-2.5 w-2.5 rounded-full sm:hidden ${device.estado === 'normal' ? 'bg-emerald-500' : device.estado === 'critico' ? 'bg-red-500' : device.estado === 'advertencia' ? 'bg-amber-500' : 'bg-slate-400'}`} title={device.estado.replace('_', ' ')} /><span className={`hidden px-2 py-1 text-[9px] font-black uppercase sm:inline-block ${stateTone[device.estado] ?? stateTone.sin_datos}`}>{device.estado.replace('_', ' ')}</span><p className="mt-1 text-[9px] text-faint">{ago(device.ultimoContactoEn)}</p></div></div>
    {motorMode ? <div className="bg-surface p-3 sm:p-4"><div className="flex flex-col items-stretch gap-3 min-[400px]:flex-row min-[400px]:items-center"><div className="min-w-0 flex-1"><strong className="block text-base text-content">Motor / persiana</strong><span className="mt-1 block text-[10px] font-bold uppercase text-faint">Modo motor confirmado por eWeLink</span></div>{canControl ? <button onClick={() => onCommand()} className="min-h-12 w-full bg-cyan-700 px-4 text-xs font-black uppercase text-white min-[400px]:w-auto">Operar motor</button> : editable && remoteEnabled && operable ? <button onClick={onSettings} className="min-h-12 w-full bg-amber-500/10 px-3 text-[10px] font-black uppercase text-amber-700 min-[400px]:w-auto">Habilitar</button> : null}</div></div> : channels.length > 0 ? <div className="grid gap-px bg-line sm:grid-cols-2">{channels.map((channel, index) => { const metrics = channelMetrics(device, index + 1); const power = metrics.find((item) => /^(actpow|power)/i.test(item.clave)); const canal = Number(channel.clave.slice(7)) - 1; const nombre = channel.nombre || `Canal ${index + 1}`; const load = loadLabels[channel.uso || 'carga']; return <div key={channel.id} className="min-w-0 bg-surface p-3 sm:p-4"><div className="flex min-w-0 flex-col items-stretch gap-3 min-[400px]:flex-row min-[400px]:items-center"><div className="min-w-0 flex-1"><strong className="block break-words text-base leading-tight text-content">{nombre}</strong><span className="mt-0.5 block break-words text-[10px] font-bold uppercase text-faint">{load}</span><span className={`mt-1 inline-block break-words text-[10px] font-black uppercase ${channel.valorBooleano ? 'text-amber-600' : 'text-faint'}`}>{channel.valorBooleano ? 'Encendida' : 'Apagada'}{channel.valorBooleano && power ? ` · ${valueOf(power)}` : ''}</span></div>{canControl ? <button onClick={() => onCommand({ canal, encendido: !channel.valorBooleano, nombre })} className={`min-h-12 w-full px-4 text-xs font-black uppercase min-[400px]:w-auto min-[400px]:min-w-24 ${channel.valorBooleano ? 'border border-line text-content' : 'bg-cyan-700 text-white'}`}>{channel.valorBooleano ? 'Apagar' : 'Encender'}</button> : editable && remoteEnabled && operable ? <button onClick={onSettings} className="min-h-12 w-full bg-amber-500/10 px-3 text-[10px] font-black uppercase text-amber-700 min-[400px]:w-auto">Habilitar</button> : null}</div></div>; })}</div> : <div className="p-4 text-sm text-muted">{device.tipo === 'puente_rf' ? 'Puente RF conectado; se conserva como acceso.' : 'Equipo de monitoreo sin salidas de mando.'}</div>}
    {expanded && <div className="border-t border-line">{channels.length > 0 && !motorMode && <div className="border-b border-line p-3 sm:p-4"><div className="mb-3"><h3 className="text-xs font-black uppercase tracking-wider text-content">Medición eléctrica por salida</h3><p className="mt-1 text-[11px] text-faint">Sólo se atribuye consumo actual a las salidas encendidas.</p></div><div className="grid gap-3 sm:grid-cols-2">{channels.map((channel, index) => <ChannelTelemetry key={channel.id} device={device} channel={channel} index={index} onHistory={onHistory} />)}</div></div>}<div className="p-3 sm:p-4"><h3 className="mb-3 text-xs font-black uppercase tracking-wider text-content">Estado del equipo</h3><div className="grid grid-cols-2 gap-px overflow-hidden border border-line bg-line sm:grid-cols-3">{shown.length ? shown.map((variable) => <MeasurementTile key={variable.id} device={device} variable={variable} onHistory={onHistory} />) : <div className="col-span-full bg-surface p-4 text-center text-xs text-faint">No hay otras mediciones operativas.</div>}</div></div><div className="grid grid-cols-2 gap-px border-t border-line bg-line text-[10px] uppercase text-faint sm:flex sm:items-center sm:gap-4 sm:bg-surface sm:p-3">{showBattery && device.bateria != null && <span className="bg-surface p-3 sm:p-0">Batería {device.bateria}%</span>}{showSignal && device.rssi != null && <span className="bg-surface p-3 sm:p-0">Señal {device.rssi} dBm</span>}<button onClick={onExport} className="min-h-11 bg-surface px-3 font-black sm:ml-auto"><Download size={13} className="mr-1 inline" />Exportar</button>{editable && <button onClick={onSettings} className="min-h-11 bg-surface px-3 font-black"><Settings2 size={13} className="mr-1 inline" />Configurar</button>}</div></div>}
